...
import { View, Text, StyleSheet, Image, Dimensions, TextInput, SafeAreaView, TouchableOpacity,ScrollView } from 'react-native';
...
export default class Messages extends Component {
  ...
  searchbyText(keyword){
    ...
    this.setState({
      newMatches : this.state.newMatches.filter(match => match.name.includes('')),
      Messages : this.state.Messages.filter(message =>  message.message.includes(''))
    })
    
  }
  render() {
    const { navigate } = this.props.navigation;
    const filtered_newMatches = this.newMatches.filter(createFilter(this.state.searchTerm, KEYS_TO_FILTERS_NEWMATCHES ))
    const filtered_Messages = this.Messages.filter(createFilter(this.state.searchTerm, KEYS_TO_FILTERS_MESSAGES ))
    return (
      <>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.main}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 12 }}>
            ...
            <Text style={{ color: colors.lightgreen, fontSize: 18, fontWeight: '600' }}>{this.newMatches.length}</Text>
            </View>
          </View>

          <View style={{ flexDirection: 'row', marginTop: 12, height: 90 }} horizontal>
            {
              filtered_newMatches.map((prop, key) => {
                return (
                  <TouchableOpacity key={prop.id} onPress={() => navigate('Chat',{user : prop.name, avatar : prop.avatar})}>
                    <View style={{ paddingRight: 30}}>
                      <Image source={prop.avatar} style={styles.user} />
                      <View style={styles.dot} />
                      <Text style={[styles.name, { marginLeft: 10 }]}>{prop.name}</Text>
                    </View>
                  </TouchableOpacity>
                )
              })
          ...
          <Text style={{ color: colors.white, fontSize: 18, fontWeight: '600' }}>{this.Messages.length}</Text>
            </View>
          </View>
          <ScrollView>
            {
                ...
                return (
                  <View style={styles.list}  key={prop.id} >
                    ...
                    <View style={styles.listborder}>
                      <Text style={{ fontSize: 20, fontFamily: "ProximaNova-Bold", color: '#666'}}>{prop.name}</Text>
